...
public class TestMaxByAggregation
{
    ...
    public void testAllRegistered()
    {
        ...
        for (Type keyType : orderableTypes) {
            for (Type valueType : metadata.getTypeManager().getTypes()) {
                assertNotNull(functionRegistry.getExactFunction(new Signature("max_by", valueType.getTypeSignature(), valueType.getTypeSignature(), keyType.getTypeSignature())));
            }
        }
    }
    ...
    public void testNull()
            throws Exception
    {
        InternalAggregationFunction doubleDouble = functionRegistry.getExactFunction(new Signature("max_by", StandardTypes.DOUBLE, StandardTypes.DOUBLE, StandardTypes.DOUBLE)).getAggregationFunction();
        assertAggregation(
                doubleDouble,
                1.0,
                null,
    ...
    public void testDoubleVarchar()
            throws Exception
    {
        InternalAggregationFunction doubleVarchar = functionRegistry.getExactFunction(new Signature("max_by", StandardTypes.VARCHAR, StandardTypes.VARCHAR, StandardTypes.DOUBLE)).getAggregationFunction();
        assertAggregation(
                doubleVarchar,
                1.0,
                "a",

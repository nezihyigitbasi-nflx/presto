...
{
    private static final Logger log = Logger.get(ShardBalancer.class);
            ...
            String connectorId)
    {
        ...
        this.executor = newScheduledThreadPool(1, daemonThreadsNamed("shard-balancer-" + connectorId));
    }

    @PostConstruct
    public void start()
    {
        if (backupAvailable && coordinator && !started.getAndSet(true)) {
            startJob();
        }
    }
    ...
    private void startJob()
    {
        executor.scheduleWithFixedDelay(() -> {
            try {
                ...
                log.error(t, "Error balancing shards");
                jobErrors.update(1);
            }
